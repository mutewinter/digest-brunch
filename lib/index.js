...
Digest = (function() {
  ...
  Digest.prototype.onCompile = function() {
    ...
    if ((_ref = this.config.env, __indexOf.call(this.options.environments, _ref) < 0) && !this.options.alwaysRun) {
  ...
  Digest.prototype._replaceReferences = function(referenceFiles, renamedFiles) {
    var contents, match, originalFilename, originalString, processedString, referenceFile, replacementFilename, replacementMap, _i, _len, _ref, _results;
    _results = [];
    for (_i = 0, _len = referenceFiles.length; _i < _len; _i++) {
      ...
      while (match !== null) {
        ...
        if (((_ref = this.options.prependHost) != null ? _ref[this.config.env] : void 0) != null) {
          replacementFilename = this.options.prependHost[this.config.env] + replacementFilename;
        }
